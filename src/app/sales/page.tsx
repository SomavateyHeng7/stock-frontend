				...
				<article className="rounded-2xl border border-border/70 bg-card p-4 shadow-sm">
					<div className="grid grid-cols-2 gap-3">
						<div className="rounded-xl border border-border/70 bg-muted/30 p-3">
							...
							<p className="text-xl font-semibold text-foreground">{salesThisWeek}</p>
						</div>
						<div className="rounded-xl border border-border/70 bg-muted/30 p-3">
							<p className="text-sm text-muted-foreground">Trend vs Last Week</p>
							<p className="text-xl font-semibold text-foreground">{salesTrendPct}%</p>
						</div>
					</div>
					...
					<h3 className="mt-4 text-base font-semibold text-foreground">Top movers</h3>
						...
						{[...enrichedProducts]
							...
							.map((item) => (
								<li key={item.id} className="flex items-center justify-between rounded-xl border border-border/70 bg-muted/20 px-3 py-2">
									...
									<span className="text-sm text-muted-foreground">Today: {item.todaySales}</span>
								</li>
							))}
					</ul>
				</article>
