...
async function sendTelegramAlert(message: string, chatId: string) {
    const response = await fetch("/api/integrations/telegram/send", {
        ...
        body: JSON.stringify({
            message,
            // Pass the user-configured chat ID; the route handler will also fall back
            // to TELEGRAM_CHAT_ID env var if this is an empty string.
            ...(chatId ? { chatId } : {}),
        }),
    });
    ...
    useEffect(() => {
        const runCheckIfDue = async () => {
            ...
            if (breachedRows.length > 0) {
                ...
                try {
                    await sendTelegramAlert(message, settings.telegramChatId);
                    pushNotification({
